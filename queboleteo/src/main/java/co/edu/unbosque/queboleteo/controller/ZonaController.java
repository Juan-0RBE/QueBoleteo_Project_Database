...
@Tag(name = "Gestión de Zonas", description = "Endpoints para la gestión de zonas")
...
public class ZonaController {
	...
	/**
	 * Genera los lugares físicos de una zona. Si la zona tiene asientos, requiere
	 * filas y asientosPorFila. Si es zona general, requiere capacidadGeneral.
	 *
	...
	public ResponseEntity<String> configurarLugares(@PathVariable Long idZona, @RequestBody ConfiguracionLugarDto dto) {
		...
		if (status == 0) {
			...
			return new ResponseEntity<>("Datos inválidos. Para zona con asientos envía filas y asientosPorFila. "
					+ "Para zona general envía capacidadGeneral.", HttpStatus.BAD_REQUEST);
		}
	}
}