...
public class ZonaConciertoService implements CRUDOperation<ZonaConciertoDTO> {
     ...
     * Resuelve las relaciones con Zona y Concierto a partir de sus IDs.
     *
     ...
     * Extrae solo los IDs de Zona y Concierto.
     *
     ...
     * Verifica que no exista ya esa combinación zona+concierto.
     *
    ...
    public int create(ZonaConciertoDTO newData) {
        // Verificamos duplicado por combinación zona+concierto
        if (newData.getIdZona() != null && newData.getIdConcierto() != null) {
            ...
            if (zona.isPresent() && concierto.isPresent()) {
                Optional<ZonaConcierto> found = zonaConciertoRepo
                        .findByZonaAndConcierto(zona.get(), concierto.get());
                if (found.isPresent()) {
                    return 1;
                }
            }
        ...
        zonaConciertoRepo.save(toEntity(newData));
        return 0;
    }
