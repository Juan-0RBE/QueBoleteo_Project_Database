...
public class UsuarioController {
	...
	public ResponseEntity<String> createUser(@RequestBody UsuarioDTO newUser) {
		...
		if (status == 0) {
			...
			return new ResponseEntity<>("El nombre de usuario ya existe",
					HttpStatus.NOT_ACCEPTABLE);

		} else if (status == 2) {
			return new ResponseEntity<>("La contraseña no cumple los requisitos",
					HttpStatus.NOT_ACCEPTABLE);
			...
			return new ResponseEntity<>("Error al crear usuario",
	...
	public ResponseEntity<UsuarioDTO> getByCorreo(
	...
	public ResponseEntity<UsuarioDTO> getByUsername(
	...
	public ResponseEntity<String> updateUser(
			@PathVariable String correo,
			@RequestBody UsuarioDTO newUser) {

		int status = usuarioServ.updateByCorreo(correo, newUser);

		if (status == 0) {
			return new ResponseEntity<>("Usuario actualizado exitosamente",
					HttpStatus.OK);
			...
			return new ResponseEntity<>("Error al actualizar",
					HttpStatus.BAD_REQUEST);
		}
	}
	...
	public ResponseEntity<String> updatePassword(
			@PathVariable String correo,
			@RequestBody UsuarioDTO newData) {

		int status = usuarioServ.updatePasswordByCorreo(correo, newData);

		if (status == 0) {
			return new ResponseEntity<>("Contraseña actualizada exitosamente",
					HttpStatus.OK);
			...
			return new ResponseEntity<>("Error al actualizar contraseña",
					HttpStatus.BAD_REQUEST);
		}
	}
	...
	public ResponseEntity<String> verifyUser(
		...
		if (usuario.getIsVerified()) {
			return new ResponseEntity<>("La cuenta ya está verificada",
					HttpStatus.OK);
		}
		...
		if (status == 0) {
			return new ResponseEntity<>("Usuario verificado exitosamente",
					HttpStatus.OK);
		}

		return new ResponseEntity<>("Error al verificar usuario",
				HttpStatus.BAD_REQUEST);
	}
	...
	public ResponseEntity<String> deleteByCorreo(
			@PathVariable String correo) {

		int status = usuarioServ.deleteByCorreo(correo);

		if (status == 0) {
			return new ResponseEntity<>("Usuario eliminado exitosamente",
					HttpStatus.OK);
		}
