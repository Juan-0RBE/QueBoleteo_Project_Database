...
public class ResenaService implements CRUDOperation<ResenaDTO> {
     ...
     * Resuelve las relaciones con Usuario (correo) y Concierto (idConcierto).
     *
     ...
     * Extrae solo el correo del usuario y el ID del concierto.
     *
     ...
     * @return 0 si fue exitoso, 1 si ese usuario ya reseñó ese concierto
     */
    @Override
    public int create(ResenaDTO newData) {
        if (newData.getCorreoUsuario() != null && newData.getIdConcierto() != null) {
            ...
            if (usuario.isPresent() && concierto.isPresent()) {
                Optional<Resena> found = resenaRepo
                        .findByUsuarioAndConcierto(usuario.get(), concierto.get());
                if (found.isPresent()) {
                    return 1;
                }
            }
