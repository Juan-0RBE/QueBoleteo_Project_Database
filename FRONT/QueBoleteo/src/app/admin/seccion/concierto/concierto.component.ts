...
// Para edición inline de precio/cantidad
  ...
  cargarConciertos(): void {
    this.conciertoService.getAll().pipe(
      catchError(() => of([]))
    ).subscribe({
      next: (data) => { this.conciertos = data ?? []; this.cdr.detectChanges(); },
    });
  }

  cargarSedes(): void {
    this.sedeService.getAllSedes().pipe(
      catchError(() => of([]))
    ).subscribe({ next: (data) => this.sedes = data ?? [] });
  }

  cargarTours(): void {
    this.tourService.getAll().pipe(
      catchError(() => of([]))
  ...
  guardar(): void {
    ...
    const body: ConciertoDTO = {
      ...this.formulario,
      ...this.formulario,
      fechaConcierto: (this.formulario.fechaConcierto as any) instanceof Date
    ...
    if (this.modoEdicion && this.conciertoEditandoId !== null) {
      this.conciertoService.update(this.conciertoEditandoId, body).subscribe({
        ...
        error: () => { this.errorMsg = 'Error al actualizar el concierto'; this.loading = false; }
      });
    } else {
      this.conciertoService.create(body).subscribe({
        ...
        error: () => { this.errorMsg = 'Error al crear el concierto'; this.loading = false; }
      });
    }
  }
  ...
  eliminar(id: number): void {
    this.conciertoService.delete(id).subscribe({
      next: () => { this.successMsg = 'Concierto eliminado'; this.cargarConciertos(); },
  ...
  togglePanelArtistas(c: ConciertoDTO): void {
    ...
    if (this.conciertoArtistaAbierto === id) { this.cerrarPanelArtistas(); return; }
    this.cerrarTodosLosPaneles();
  ...
  asociarArtista(): void {
    ...
    this.errorArtista = '';
    this.successArtista = '';
    const dto: ConArt = { idConcierto: this.conciertoArtistaAbierto, idArtista: this.artistaSeleccionado.idArtista! };
    this.conciertoService.asociarArtista(dto).subscribe({
      next: () => { this.successArtista = 'Artista asociado'; this.artistaSeleccionado = null; this.cargarArtistasAsociados(this.conciertoArtistaAbierto!); },
      error: (err) => { this.errorArtista = err.status === 406 ? 'Ese artista ya está asociado' : 'Error al asociar'; }
    });
  }
  ...
  desasociarArtista(idArtista: number): void {
    ...
    this.conciertoService.desasociarArtista(this.conciertoArtistaAbierto, idArtista).subscribe({
      next: () => { this.successArtista = 'Artista desasociado'; this.cargarArtistasAsociados(this.conciertoArtistaAbierto!); },
  ...
  togglePanelGrupos(c: ConciertoDTO): void {
    const id = c.idConcierto!;
    if (this.conciertoGrupoAbierto === id) { this.cerrarPanelGrupos(); return; }
    this.cerrarTodosLosPaneles();
  ...
  asociarGrupo(): void {
    ...
    this.errorGrupo = '';
    this.successGrupo = '';
    const dto: ConGru = { idConcierto: this.conciertoGrupoAbierto, idGrupo: this.grupoSeleccionado.idGrupo! };
    this.conciertoService.asociarGrupo(dto).subscribe({
      next: () => { this.successGrupo = 'Grupo asociado'; this.grupoSeleccionado = null; this.cargarGruposAsociados(this.conciertoGrupoAbierto!); },
      error: (err) => { this.errorGrupo = err.status === 406 ? 'Ese grupo ya está asociado' : 'Error al asociar'; }
    });
  }
  ...
  desasociarGrupo(idGrupo: number): void {
    ...
    this.conciertoService.desasociarGrupo(this.conciertoGrupoAbierto, idGrupo).subscribe({
      next: () => { this.successGrupo = 'Grupo desasociado'; this.cargarGruposAsociados(this.conciertoGrupoAbierto!); },
  ...
  togglePanelOrganizadores(c: ConciertoDTO): void {
    const id = c.idConcierto!;
    if (this.conciertoOrgAbierto === id) { this.cerrarPanelOrganizadores(); return; }
    this.cerrarTodosLosPaneles();
  ...
  asociarOrganizador(): void {
    ...
    this.errorOrg = '';
    this.successOrg = '';
    const dto: ConOrg = { idConcierto: this.conciertoOrgAbierto, nombreOrganizador: this.organizadorSeleccionado.nombreOrganizador };
    this.conciertoService.asociarOrganizador(dto).subscribe({
      next: () => { this.successOrg = 'Organizador asociado'; this.organizadorSeleccionado = null; this.cargarOrganizadoresAsociados(this.conciertoOrgAbierto!); },
      error: (err) => { this.errorOrg = err.status === 406 ? 'Ese organizador ya está asociado' : 'Error al asociar'; }
    });
  }

  desasociarOrganizador(nombreOrganizador: string): void {
    if (!this.conciertoOrgAbierto) return;
    this.conciertoService.desasociarOrganizador(this.conciertoOrgAbierto, nombreOrganizador).subscribe({
      next: () => { this.successOrg = 'Organizador desasociado'; this.cargarOrganizadoresAsociados(this.conciertoOrgAbierto!); },
  ...
  togglePanelZonas(c: ConciertoDTO): void {
    const id = c.idConcierto!;
    if (this.conciertoZonaAbierto === id) { this.cerrarPanelZonas(); return; }
    this.cerrarTodosLosPaneles();
  ...
  cargarZonasDeSede(nombreSede: string): void {
    this.sedeService.getAllZonas().pipe(
      catchError(() => of([]))
    ).subscribe({
      next: (data) => {
